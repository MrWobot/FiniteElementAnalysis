...
namespace FiniteElementAnalysis.Mesh.Parsing.Planar
{
    ...
    public static class PlanarDomainFromObjMtlHelper
    {
            ...
            NormalAxis normalAxis = NormalAxis.AutoDetermine)
        {
            ...
            NormalAxis resolvedAxis = Step2_ResolveNormalAxis(normalAxis, rawFaces, volumes);
            ...
            double nearPlaneCoordinate = Step4_FindNearPlaneCoordinate(classifiedFaces, getAxisCoordinate, toleranceMeters);

            // Step 5: Separate planar faces into volume faces (on near plane) and ignored faces (far plane)
            List<RawFace> volumeFaces = Step5_ExtractVolumeFaces(classifiedFaces, getAxisCoordinate, nearPlaneCoordinate, toleranceMeters);
            ...
            foreach (RawFace boundaryFace in boundaryFaces)
            {
                ...
                var verticesOnPlaneWithAxis = boundaryFace.Vertices
                    ...
                    .Where(o => Math.Abs(nearPlaneCoordinate - o.AxisCoordinate) <= toleranceMeters)
                    .Take(2)
                    .ToArray();
        ...
        private static NormalAxis Step2_ResolveNormalAxis(NormalAxis requested, List<RawFace> rawFaces, VolumesCollection volumes)
        {
            //TODO make this instead look for faces with materials that match up to volumes to identify the axis. 
            if (requested != NormalAxis.AutoDetermine)
                return requested;
            var volumeFaces = rawFaces.Where(f => f.MaterialName != null && volumes.TryGetVolumeByName(f.MaterialName!) != null);
            var groupedByNormalAxis = volumeFaces.Select(f => new { face = f, normalAxis = ComputeRawFaceNormalAxis(f) }).GroupBy(o => o.normalAxis).Select(g => g.ToArray());
            if (groupedByNormalAxis.Count() > 1)
            {
                var wrongAxisGroups = groupedByNormalAxis.OrderByDescending(g => g.Count()).Skip(1);
                throw new Exception("Failed to determine normal axis as volume faces had different corresponding normal axis. They should share the same normal axis!");
            }
            return groupedByNormalAxis.First().First().normalAxis;
            /*
        ...
        private static NormalAxis ComputeRawFaceNormalAxis(RawFace rawFace)
        {
            var faceNormal = ComputeFaceNormal(rawFace);
            return faceNormal.X > faceNormal.Y ? faceNormal.Z > faceNormal.X ? NormalAxis.Z : NormalAxis.Z : faceNormal.Z > faceNormal.Y ? NormalAxis.Z : NormalAxis.Y;
        }
